    ...
    try {
      ...
      if (!response.ok) {
        return rejectWithValue(
          "Ошибка при получении списка файлов пользователя."
        );
      }

      return await response.json();
    } catch (error) {
      return rejectWithValue("Ошибка со стороны сервера.");
    }
  }
);
...
export const changeFile = createAsyncThunk(
  ...
  async (fileData: IChangeFileData, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        return rejectWithValue(
          "Ошибка при изменении данных о файле."
        );
      }
    } catch (error) {
      return rejectWithValue("Ошибка со стороны сервера.");
...
export const getFileLink = createAsyncThunk(
  ...
  async (fileId: number, { rejectWithValue }) => {
    try {
      ...
      if (!response.ok) {
        return rejectWithValue(
          "Ошибка при получении специальной ссылки на файл."
        );
      }
